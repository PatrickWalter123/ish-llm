...
* `ish/engines`: Engine protocol, context snapshots, event types, registry, FakeStreamingEngine, and LiteLLM LoopEngine
...
* `ish/services/tasks.py`: Task lifecycle and conversation snapshot cloning
* `ish/services/conversation.py`: append-only JSONL conversation events
* `ish/services/run_manager.py`: TaskRuntime queues, serial workers, Run lifecycle, context assembly, interrupt, shutdown, and recovery
* `ish/services/runs.py`: atomic Run metadata persistence
* `ish/services/steps.py`: StepManager lifecycle and StepEventRecorder translation
...
TaskManager also rejects persisted active Run state. No cross-process locking
...
Soft deletion currently marks metadata in place. Task clones normalize turn
...
`engines/litellm_stream.py`. Each request has a dedicated daemon thread that
